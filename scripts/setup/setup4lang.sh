...
MAGYARLANC_DIR=$LOCAL_4LANG_BIN_DIR/magyarlanc
if [ ! -f "$LOCAL_4LANG_BIN_DIR/$MAGYARLANC_FILENAME" ]; then
  echo "Copying magyarlanc"
  mkdir $MAGYARLANC_DIR
  cp $LOCAL_4LANG_DOWNLOAD_DIR/$MAGYARLANC_FILENAME $MAGYARLANC_DIR
else
...
if [ ! -f ./export.done ]; then
  ...
  echo "export MAGYARLANCPATH=$PWD/$MAGYARLANC_DIR"
  sudo echo "export MAGYARLANCPATH=$PWD/$MAGYARLANC_DIR" >> ~/.bashrc
  echo "MAGYARLANCPATH=$PWD/$MAGYARLANC_DIR" >> ./4lang.env
